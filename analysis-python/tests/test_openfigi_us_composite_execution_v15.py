...
from equity_analysis.fundamental_value.openfigi_us_composite_execution_v15 import (
    ...
    UsCompositeExecutionStop,
    UsCompositePhaseAuthorization,
    execute_openfigi_us_composite_diagnostic_v15,
    git_safe_us_composite_execution_summary_v1,
    seal_storage_backed_us_composite_acceptance_v1,
    seal_us_composite_phase_authorization_v1,
    us_composite_run_root_v1,
    validate_storage_backed_us_composite_acceptance_v1,
    validate_us_composite_execution_result_v1,
    validate_us_composite_replay_verification_v1,
...
def _ambiguous_responses() -> tuple[TransportResponse, ...]:
    plan = build_frozen_us_composite_plan_v1()
    responses = list(_successful_responses())
    payload = json.loads(responses[0].body)
    first_job = plan.requests[0].jobs[0]
    payload[0]["data"].append(
        _candidate(first_job.expected_symbol, first_job.member_ordinal, variant=7)
    )
    responses[0] = replace(
        responses[0],
...
def _authorization(
    run_id: str, *, network_authorized: bool = True
) -> UsCompositePhaseAuthorization:
    return seal_us_composite_phase_authorization_v1(
        build_frozen_us_composite_plan_v1(),
        run_id=run_id,
...
def _execute_success(
    ...
    *,
    run_id: str,
) -> tuple[UsCompositePhaseAuthorization, tuple[TransportResponse, ...], _Clock]:
    ...
    result = execute_openfigi_us_composite_diagnostic_v15(
        plan,
        authorization,
...
def _assert_stop(code: str, callable_) -> None:
    with pytest.raises(UsCompositeExecutionStop, match=code) as caught:
        callable_()
...
def test_authorization_is_separate_plan_bound_and_network_false_by_default(tmp_path: Path) -> None:
    ...
    authorization = _authorization(
        "20260802T180000Z-STAGE8C-DIAGNOSTIC-V15-001",
    ...
    assert plan.content_hash == ("F7F2E728FB4B91D9FA7E9B7F63B5E38259655B7B64D798BD63BE54DCA187FB40")
    assert CONTROLLER_AUTHORITY_CONTENT_HASH not in {
        "653B77CFF237DEE95A38518D6EF8B8CFF359A60CA1F893D0549D7254CAC78252",
    ...
    _assert_stop(
        "US_COMPOSITE_EXECUTION_NETWORK_NOT_AUTHORIZED",
        lambda: execute_openfigi_us_composite_diagnostic_v15(
            plan,
            authorization,
    ...
    _assert_stop(
        "US_COMPOSITE_EXECUTION_RUN_ID_INVALID",
        lambda: _authorization("20260802T141600Z-STAGE8C-OPENFIGI-V14-001"),
    )
    _assert_stop(
        "US_COMPOSITE_EXECUTION_CONTROLLER_AUTHORITY_HASH_REQUIRED",
        lambda: seal_us_composite_phase_authorization_v1(
            plan,
            run_id="20260802T180001Z-STAGE8C-DIAGNOSTIC-V15-001",
            test_only=True,
...
def test_authorization_requires_exact_scalar_runtime_types(mutation) -> None:
    plan = build_frozen_us_composite_plan_v1()
    authorization = mutation(_authorization("20260802T180050Z-STAGE8C-DIAGNOSTIC-V15-050"))
    _assert_stop(
        "US_COMPOSITE_EXECUTION_AUTHORIZATION_BINDING_DRIFT",
        lambda: execute_openfigi_us_composite_diagnostic_v15(
            plan,
            authorization,
...
def test_production_requires_exact_transport_and_real_clock_functions(tmp_path: Path) -> None:
    ...
    authorization = seal_us_composite_phase_authorization_v1(
        plan,
        run_id="20260802T180051Z-STAGE8C-DIAGNOSTIC-V15-051",
        accepted_controller_authority_content_hash=(CONTROLLER_AUTHORITY_CONTENT_HASH),
        test_only=False,
        network_authorized=True,
    )
    ...
    _assert_stop(
        "US_COMPOSITE_EXECUTION_PRODUCTION_TRANSPORT_TYPE_INVALID",
        lambda: execute_openfigi_us_composite_diagnostic_v15(
            plan,
            authorization,
            storage_root=tmp_path,
    ...
    _assert_stop(
        "US_COMPOSITE_EXECUTION_PRODUCTION_CLOCK_INJECTION_BLOCKED",
        lambda: execute_openfigi_us_composite_diagnostic_v15(
            plan,
            authorization,
...
def test_private_storage_marker_is_required_before_any_send(tmp_path: Path) -> None:
    plan = build_frozen_us_composite_plan_v1()
    authorization = _authorization("20260802T180052Z-STAGE8C-DIAGNOSTIC-V15-052")
...
def test_two_send_success_pacing_private_checkpoint_and_exact_zero_send_replay(
    ...
    plan = build_frozen_us_composite_plan_v1()
    run_id = "20260802T180100Z-STAGE8C-DIAGNOSTIC-V15-002"
    authorization = _authorization(run_id)
    responses = _successful_responses()
    clock = _Clock()
    transport = _Transport(responses, clock)

    first = execute_openfigi_us_composite_diagnostic_v15(
        plan,
        authorization,
        storage_root=tmp_path,
        transport=transport,
    ...
    assert len(transport.requests) == 2
    assert transport.send_micros[1] - transport.send_micros[0] >= (OPENFIGI_PACING_INTERVAL_MICROS)
    assert clock.sleep_calls == [2.4]
    summary = git_safe_us_composite_execution_summary_v1(first)
    serialized_summary = json.dumps(summary)
    assert "responses" not in summary
    assert "BBG000000010" not in serialized_summary
    run_root = us_composite_run_root_v1(tmp_path, authorization)
    ...
    replay = execute_openfigi_us_composite_diagnostic_v15(
        plan,
        authorization,
        storage_root=tmp_path,
        transport=no_send,
    ...
    _assert_stop(
        "US_COMPOSITE_EXECUTION_RESULT_BINDING_DRIFT",
        lambda: validate_us_composite_execution_result_v1(
...
def test_storage_replay_builds_review_and_acceptance_with_receipt_set_hash(
    ...
    authorization, responses, _clock = _execute_success(
        tmp_path,
        run_id="20260802T180200Z-STAGE8C-DIAGNOSTIC-V15-003",
    )
    review = build_us_composite_review_v1(plan, responses)
    ...
    verification2, diagnostic, storage_acceptance = seal_storage_backed_us_composite_acceptance_v1(
        plan,
        authorization,
        review,
        storage_root=tmp_path,
    ...
    assert storage_acceptance.diagnostic_only is True
    assert storage_acceptance.post_predecessor_observation is True
    assert storage_acceptance.durable_identity_authorized is False
    assert storage_acceptance.remainder_authorized is False
    assert storage_acceptance.evidence_upgrade_authorized is False
    assert storage_acceptance.operating_mic_binding_status == ("REQUIRES_SEC_CORROBORATION")
    serialized_acceptance = json.dumps(storage_acceptance.__dict__, sort_keys=True)
    assert "BBG" not in serialized_acceptance
    assert "GOOG" not in serialized_acceptance
    assert "FOX" not in serialized_acceptance
    assert "MSFT" not in serialized_acceptance
    assert (
        storage_acceptance.checkpoint_receipt_set_hash == verification.checkpoint_receipt_set_hash
    )
    validate_storage_backed_us_composite_acceptance_v1(
        plan,
        authorization,
    ...
    _assert_stop(
        "US_COMPOSITE_EXECUTION_REPLAY_VERIFICATION_DRIFT",
        lambda: validate_us_composite_replay_verification_v1(
            replace(verification, replayed_physical_request_count=2.0)
        ),
    )
    _assert_stop(
        "US_COMPOSITE_EXECUTION_STORAGE_ACCEPTANCE_DRIFT",
        lambda: validate_storage_backed_us_composite_acceptance_v1(
            plan,
            authorization,
            review,
            verification,
    ...
    _assert_stop(
        "US_COMPOSITE_EXECUTION_ACCEPTED_DECISION_CODE_INVALID",
        lambda: seal_storage_backed_us_composite_acceptance_v1(
            plan,
            authorization,
            review,
            storage_root=tmp_path,
            accepted=True,
            decision_code="US_COMPOSITE_ARBITRARY_SUCCESS",
        ),
    )


def test_storage_backed_rejection_requires_failed_gate_and_fixed_code(
    tmp_path: Path,
) -> None:
    plan = build_frozen_us_composite_plan_v1()
    authorization = _authorization(
        "20260802T180250Z-STAGE8C-DIAGNOSTIC-V15-REJECT",
    )
    responses = _ambiguous_responses()
    clock = _Clock()
    execute_openfigi_us_composite_diagnostic_v15(
        plan,
        authorization,
    ...
    verification, diagnostic, storage_acceptance = seal_storage_backed_us_composite_acceptance_v1(
        plan,
        authorization,
        review,
        storage_root=tmp_path,
    ...
    assert storage_acceptance.accepted is False
    assert storage_acceptance.decision_code == REJECTED_DECISION_CODE
    assert storage_acceptance.diagnostic_only is True
    assert storage_acceptance.durable_identity_authorized is False
    assert storage_acceptance.remainder_authorized is False
    assert storage_acceptance.evidence_upgrade_authorized is False
    _assert_stop(
        "US_COMPOSITE_EXECUTION_REJECTED_DECISION_CODE_INVALID",
        lambda: seal_storage_backed_us_composite_acceptance_v1(
            plan,
            authorization,
            review,
            storage_root=tmp_path,
            accepted=False,
            decision_code="US_COMPOSITE_EXECUTION_ARBITRARY_REJECTION",
        ),
    )
...
def test_forged_self_consistent_in_memory_review_cannot_authorize(
    ...
    authorization, responses, _clock = _execute_success(
        tmp_path,
        run_id="20260802T180300Z-STAGE8C-DIAGNOSTIC-V15-004",
    )
    stored_review = build_us_composite_review_v1(plan, responses)
    alternate_review = build_us_composite_review_v1(
        plan,
        _successful_responses(variant=1),
    )
    assert alternate_review != stored_review
    assert alternate_review.unique_primary_count == 6

    _assert_stop(
        "US_COMPOSITE_EXECUTION_STORAGE_REVIEW_REPLAY_DRIFT",
        lambda: seal_storage_backed_us_composite_acceptance_v1(
            plan,
            authorization,
            alternate_review,
            storage_root=tmp_path,
            accepted=True,
            decision_code="US_COMPOSITE_EXECUTION_COMPLETE_CONVERGENT",
        ),
    )
...
def test_second_send_uncertainty_leaves_unmatched_intent_and_never_retries(
    ...
    plan = build_frozen_us_composite_plan_v1()
    authorization = _authorization("20260802T180400Z-STAGE8C-DIAGNOSTIC-V15-005")
    clock = _Clock()
    ...
    no_send = _NoSendTransport()
    _assert_stop(
        "US_COMPOSITE_EXECUTION_UNMATCHED_INTENT_STOP",
        lambda: execute_openfigi_us_composite_diagnostic_v15(
            plan,
            authorization,
            storage_root=tmp_path,
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            ...
            "US_COMPOSITE_EXECUTION_AUTHORIZATION_CONTENT_HASH_DRIFT",
        ),
    ],
)
def test_forged_authorization_stops_before_transport(
    ...
    plan = build_frozen_us_composite_plan_v1()
    authorization = mutation(_authorization("20260802T180500Z-STAGE8C-DIAGNOSTIC-V15-006"))
    transport = _NoSendTransport()
    _assert_stop(
        code,
        lambda: execute_openfigi_us_composite_diagnostic_v15(
...
def test_forged_plan_stops_before_transport(tmp_path: Path) -> None:
    ...
    authorization = _authorization("20260802T180600Z-STAGE8C-DIAGNOSTIC-V15-007")
    transport = _NoSendTransport()
    _assert_stop(
        "US_COMPOSITE_EXECUTION_PLAN_INVALID",
        lambda: execute_openfigi_us_composite_diagnostic_v15(
            forged,
            authorization,
            storage_root=tmp_path,
...
def test_active_execution_lease_stops_before_transport(tmp_path: Path) -> None:
    plan = build_frozen_us_composite_plan_v1()
    authorization = _authorization("20260802T180700Z-STAGE8C-DIAGNOSTIC-V15-008")
    run_root = us_composite_run_root_v1(tmp_path, authorization)
...
def test_pacing_clock_that_does_not_advance_stops_before_second_intent(
    ...
    plan = build_frozen_us_composite_plan_v1()
    authorization = _authorization("20260802T180800Z-STAGE8C-DIAGNOSTIC-V15-009")
    clock = _Clock()
...
def test_partial_resume_after_monotonic_clock_regression_stops_without_send(
    ...
    authorization, _responses_value, clock = _execute_success(
        tmp_path,
        run_id="20260802T180900Z-STAGE8C-DIAGNOSTIC-V15-010",
    )
    run_root = us_composite_run_root_v1(tmp_path, authorization)
    second = plan.requests[1].request_identity
...
def test_checkpoint_event_manifest_path_and_orphan_drift_all_stop_replay(
    ...
    for ordinal, scenario in enumerate(scenarios, 10):
        ...
        root.mkdir()
        run_id = f"20260802T18{ordinal:02d}00Z-STAGE8C-DIAGNOSTIC-V15-{ordinal:03d}"
        authorization, _responses_value, _clock = _execute_success(root, run_id=run_id)
        run_root = us_composite_run_root_v1(root, authorization)
        ...
        if scenario == "checkpoint":
            ...
            expected = "US_COMPOSITE_EXECUTION_COMPLETED_DETAIL_DRIFT"
        elif scenario == "event":
            ...
            expected = "US_COMPOSITE_EXECUTION_JOURNAL_EVENT_CHAIN_DRIFT"
        elif scenario == "manifest":
            manifest = run_root / "plan-authorization.json"
            value = json.loads(manifest.read_text(encoding="utf-8"))
            value["retryLimit"] = 1
            manifest.write_text(json.dumps(value), encoding="utf-8")
            expected = "US_COMPOSITE_EXECUTION_IMMUTABLE_MANIFEST_DRIFT"
        elif scenario == "path":
            _rewrite_event(
                completed,
                lambda value: value["detail"].__setitem__("checkpointPath", "../escape.bin"),
            )
            expected = "US_COMPOSITE_EXECUTION_UNSAFE_CHECKPOINT_PATH"
        elif scenario == "orphan":
            (run_root / "_private" / "checkpoints" / f"{'0' * 64}.bin").write_bytes(b"x")
            expected = "US_COMPOSITE_EXECUTION_CHECKPOINT_ORPHAN_OR_PATH_DRIFT"
        else:
            completed.rename(_event_path(run_root, request_identity, 2, "FAILED"))
            _rewrite_event(
                _event_path(run_root, request_identity, 2, "FAILED"),
                lambda value: value.__setitem__("state", "FAILED"),
            )
            (run_root / "_private" / "checkpoints" / f"{request_identity}.bin").unlink()
            expected = "US_COMPOSITE_EXECUTION_FAILED_REQUEST_STOP"
        transport = _NoSendTransport()
...
@pytest.mark.parametrize(
    ...
    [
        (
            ...
            "US_COMPOSITE_EXECUTION_HTTP_STATUS_INVALID",
        ),
        (
            TransportResponse(
                200,
            ...
            "US_COMPOSITE_EXECUTION_RESPONSE_BODY_TOO_LARGE",
        ),
        (
            TransportResponse(200, (("content-type", "text/plain"),), b"[]"),
            "US_COMPOSITE_EXECUTION_RESPONSE_CONTENT_TYPE_INVALID",
        ),
    ],
)
def test_http_status_and_response_size_are_bounded_after_intent(
    ...
    plan = build_frozen_us_composite_plan_v1()
    authorization = _authorization(f"20260802T181900Z-STAGE8C-DIAGNOSTIC-V15-{code[-3:]}")
    clock = _Clock()
...
def test_run_path_orphan_stops_before_transport(tmp_path: Path) -> None:
    plan = build_frozen_us_composite_plan_v1()
    authorization = _authorization("20260802T182000Z-STAGE8C-DIAGNOSTIC-V15-020")
    run_root = us_composite_run_root_v1(tmp_path, authorization)
    run_root.mkdir(parents=True)
    (run_root / "unexpected.txt").write_text("orphan", encoding="utf-8")
    transport = _NoSendTransport()
    _assert_stop(
        "US_COMPOSITE_EXECUTION_MANIFEST_MISSING_WITH_STATE",
        lambda: execute_openfigi_us_composite_diagnostic_v15(
            plan,
            authorization,
            storage_root=tmp_path,
...
def test_execution_artifacts_stay_under_private_execution_contract_root(
    ...
    authorization, _responses_value, _clock = _execute_success(
        tmp_path,
        run_id="20260802T182100Z-STAGE8C-DIAGNOSTIC-V15-021",
    )
    run_root = us_composite_run_root_v1(tmp_path, authorization)
