"""Offline-tested execution boundary for the frozen OpenFIGI v1.4 diagnostic.
...
EXECUTION_CONTRACT_VERSION = "FV-STAGE8C-OPENFIGI-DIAGNOSTIC-EXECUTION-v1.0.0"
AUTHORIZATION_VERSION = "FV-STAGE8C-OPENFIGI-DIAGNOSTIC-AUTHORIZATION-v1.0.0"
MANIFEST_VERSION = "FV-STAGE8C-OPENFIGI-DIAGNOSTIC-MANIFEST-v1.0.0"
JOURNAL_VERSION = "FV-STAGE8C-OPENFIGI-DIAGNOSTIC-JOURNAL-v1.0.0"
SUMMARY_VERSION = "FV-STAGE8C-OPENFIGI-DIAGNOSTIC-SUMMARY-v1.0.0"
REJECTED_DECISION_CODE = "DIAGNOSTIC_REJECTED_GATE_NOT_MET"
AUTHORITY_BASIS = "USER_EXPLICIT_BROAD_FUTURE_PROVIDER_AUTHORIZATION_2026_08_02"
CONTROLLER_AUTHORITY_VERSION = (
    "FV-STAGE8C-OPENFIGI-DIAGNOSTIC-CONTROLLER-AUTHORITY-v1.0.0"
)
CONTROLLER_AUTHORITY_CONTENT_HASH = (
    "E83DE697C84381B97FC08DA612D1C0B62A86102710548F24DDF6DBB7CE43F8F7"
)
PREDECESSOR_CANARY_RUN_ID = "20260802T124156Z-STAGE8C-OPENFIGI-V13-002"
...
class DiagnosticExecutionStop(RuntimeError):
...
@dataclass(frozen=True)
class DiagnosticReplayVerification:
    run_id: str
    plan_content_hash: str
    authorization_content_hash: str
...
@dataclass(frozen=True)
class StorageBackedDiagnosticAcceptance:
    run_id: str
    plan_content_hash: str
    authorization_content_hash: str
...
def _controller_authority_body() -> dict[str, object]:
    return {
        ...
        "executionScope": "DIAGNOSTIC_ONLY_NO_REMAINDER",
    }


def _authorization_body(value: PhaseAuthorization, *, include_hash: bool) -> dict[str, object]:
...
def seal_phase_authorization_v1(
    plan: DiagnosticPlan,
    *,
    run_id: str,
    ...
    if accepted_controller_authority_content_hash != CONTROLLER_AUTHORITY_CONTENT_HASH:
        raise DiagnosticExecutionStop("DIAGNOSTIC_CONTROLLER_AUTHORITY_HASH_REQUIRED")
    provisional = PhaseAuthorization(
    ...
    validate_phase_authorization_v1(plan, result)
    return result


def validate_phase_authorization_v1(
    plan: DiagnosticPlan, authorization: PhaseAuthorization
) -> None:
    _validate_plan(plan)
    if type(authorization) is not PhaseAuthorization:
        raise DiagnosticExecutionStop("DIAGNOSTIC_AUTHORIZATION_TYPE_INVALID")
    if (
        type(authorization.run_id) is not str
        or _RUN_ID.fullmatch(authorization.run_id) is None
        or authorization.run_id == PREDECESSOR_CANARY_RUN_ID
    ):
        raise DiagnosticExecutionStop("DIAGNOSTIC_RUN_ID_INVALID")
    if type(authorization.network_authorized) is not bool:
        raise DiagnosticExecutionStop("DIAGNOSTIC_NETWORK_AUTHORIZATION_TYPE_INVALID")
    if (
        ...
        or authorization.controller_authority_content_hash
        != CONTROLLER_AUTHORITY_CONTENT_HASH
        ...
        raise DiagnosticExecutionStop("DIAGNOSTIC_AUTHORIZATION_BINDING_DRIFT")
    if authorization.content_hash != canonical_hash(
        _authorization_body(authorization, include_hash=False)
    ):
        raise DiagnosticExecutionStop("DIAGNOSTIC_AUTHORIZATION_CONTENT_HASH_DRIFT")
...
def _validate_plan(plan: DiagnosticPlan) -> None:
    if canonical_hash(_controller_authority_body()) != CONTROLLER_AUTHORITY_CONTENT_HASH:
        raise DiagnosticExecutionStop("DIAGNOSTIC_CONTROLLER_AUTHORITY_FROZEN_HASH_DRIFT")
    try:
        validate_diagnostic_plan(plan)
    except (DiagnosticStop, TypeError, ValueError) as error:
        raise DiagnosticExecutionStop("DIAGNOSTIC_EXECUTION_PLAN_INVALID") from error
    if (
        plan.content_hash != FROZEN_PLAN_CONTENT_HASH
        or plan.physical_request_count != 2
        or plan.logical_job_count != 10
        or plan.retry_limit != 0
        or plan.network_authorized is not False
    ):
        raise DiagnosticExecutionStop("DIAGNOSTIC_EXECUTION_PLAN_NOT_FROZEN")
...
def _assert_no_symlink(path: Path, *, stop: Path) -> None:
    ...
    while True:
        if current.exists() and current.is_symlink():
            raise DiagnosticExecutionStop("DIAGNOSTIC_STORAGE_SYMLINK_STOP")
...
def _validated_storage_root(storage_root: Path, *, test_only: bool) -> Path:
    if not isinstance(storage_root, Path):
        raise DiagnosticExecutionStop("DIAGNOSTIC_STORAGE_ROOT_TYPE_INVALID")
    try:
        return validate_private_storage_root(storage_root, test_only=test_only)
    except AcquisitionStop as error:
        raise DiagnosticExecutionStop(error.code) from error


def diagnostic_run_root_v1(storage_root: Path, authorization: PhaseAuthorization) -> Path:
    ...
    if type(authorization) is not PhaseAuthorization or _RUN_ID.fullmatch(
        authorization.run_id
    ) is None or authorization.run_id == PREDECESSOR_CANARY_RUN_ID:
        raise DiagnosticExecutionStop("DIAGNOSTIC_RUN_ID_INVALID")
...
def _wire_binding(wire: ProviderWireRequest) -> dict[str, object]:
    if type(wire) is not ProviderWireRequest or type(wire.headers) is not tuple:
        raise DiagnosticExecutionStop("DIAGNOSTIC_WIRE_TYPE_INVALID")
    if (
        ...
        raise DiagnosticExecutionStop("DIAGNOSTIC_WIRE_BINDING_DRIFT")
    return {
...
def _validate_transport_boundary(
    ...
    if authorization.test_only:
        if (
            ...
            raise DiagnosticExecutionStop("DIAGNOSTIC_TEST_TRANSPORT_BOUNDARY_DRIFT")
        return
    if type(transport) is not StdlibAcquisitionHttpTransport:
        raise DiagnosticExecutionStop("DIAGNOSTIC_PRODUCTION_TRANSPORT_TYPE_INVALID")
    if (
        ...
        or any(
            transport._target_url(wire, eodhd_api_key=None)
            != PROVIDER_ORIGIN + wire.endpoint_path
            for wire in wires
        )
    ):
        raise DiagnosticExecutionStop("DIAGNOSTIC_PRODUCTION_TRANSPORT_BOUNDARY_DRIFT")
...
def _manifest_body(
    ...
    body: dict[str, object] = {
        ...
        "controllerAuthorityContentHash": (
            authorization.controller_authority_content_hash
        ),
...
def _validated_headers(
    ...
    if type(headers) is not tuple:
        raise DiagnosticExecutionStop("DIAGNOSTIC_RESPONSE_HEADERS_MUST_BE_TUPLE")
    if any(
        ...
        raise DiagnosticExecutionStop("DIAGNOSTIC_RESPONSE_HEADER_INVALID")
    if len({item[0] for item in headers}) != len(headers) or headers != tuple(sorted(headers)):
        raise DiagnosticExecutionStop("DIAGNOSTIC_RESPONSE_HEADERS_NOT_CANONICAL")
    content_type = dict(headers).get("content-type")
    if (
        content_type is None
        or content_type.split(";", 1)[0].strip().lower() != "application/json"
    ):
        raise DiagnosticExecutionStop("DIAGNOSTIC_RESPONSE_CONTENT_TYPE_INVALID")
    return headers


def _response_binding(response: TransportResponse) -> dict[str, object]:
    if type(response) is not TransportResponse:
        raise DiagnosticExecutionStop("DIAGNOSTIC_TRANSPORT_RESPONSE_TYPE_INVALID")
    if type(response.status_code) is not int or response.status_code != 200:
        raise DiagnosticExecutionStop("DIAGNOSTIC_HTTP_STATUS_INVALID")
    headers = _validated_headers(response.headers)
    if type(response.body) is not bytes:
        raise DiagnosticExecutionStop("DIAGNOSTIC_RESPONSE_BODY_MUST_BE_BYTES")
    if len(response.body) > MAX_RESPONSE_BODY_BYTES:
        raise DiagnosticExecutionStop("DIAGNOSTIC_RESPONSE_BODY_TOO_LARGE")
    return {
...
class _ExecutionJournal:
    ...
    def _write_or_verify_manifest(self) -> None:
        ...
        if path.exists():
            try:
                ...
                raise DiagnosticExecutionStop("DIAGNOSTIC_MANIFEST_UNREADABLE") from error
            if actual != expected:
                raise DiagnosticExecutionStop("DIAGNOSTIC_IMMUTABLE_MANIFEST_DRIFT")
            return
        existing = tuple(item.name for item in self._run_root.iterdir() if item.name != ".lock")
        if existing:
            raise DiagnosticExecutionStop("DIAGNOSTIC_MANIFEST_MISSING_WITH_STATE")
        try:
            _atomic_json_create(path, expected)
        except FileExistsError as error:
            raise DiagnosticExecutionStop("DIAGNOSTIC_MANIFEST_CREATE_RACE") from error
    ...
    def _audit_top_level(self) -> None:
        ...
        for item in self._run_root.iterdir():
            if item.name not in permitted or item.is_symlink():
                raise DiagnosticExecutionStop("DIAGNOSTIC_RUN_PATH_OR_ORPHAN_DRIFT")
    ...
    def _audit_request_directories(self) -> None:
        ...
        if not self._journal_root.is_dir() or self._journal_root.is_symlink():
            raise DiagnosticExecutionStop("DIAGNOSTIC_JOURNAL_ROOT_INVALID")
        for path in self._journal_root.iterdir():
            if (
                not path.is_dir()
                or path.is_symlink()
                or path.name not in self._wire_by_identity
            ):
                raise DiagnosticExecutionStop("DIAGNOSTIC_JOURNAL_REQUEST_ORPHAN_DRIFT")
    ...
    def _audit_private_root(self) -> None:
        ...
        if not private_root.is_dir() or private_root.is_symlink():
            raise DiagnosticExecutionStop("DIAGNOSTIC_PRIVATE_ROOT_INVALID")
        if any(item.name != "checkpoints" for item in private_root.iterdir()):
            raise DiagnosticExecutionStop("DIAGNOSTIC_PRIVATE_ORPHAN_DRIFT")
    ...
    def events(self, wire: ProviderWireRequest) -> tuple[dict[str, object], ...]:
        ...
        for expected_sequence, path in enumerate(paths, start=1):
            ...
            if not path.is_file() or path.is_symlink() or matched is None:
                raise DiagnosticExecutionStop("DIAGNOSTIC_JOURNAL_UNEXPECTED_ARTIFACT")
            try:
                event = json.loads(path.read_text(encoding="utf-8"))
            except (OSError, ValueError) as error:
                raise DiagnosticExecutionStop("DIAGNOSTIC_JOURNAL_EVENT_UNREADABLE") from error
            ...
            if (
                ...
                raise DiagnosticExecutionStop("DIAGNOSTIC_JOURNAL_EVENT_CHAIN_DRIFT")
            events.append(event)
        ...
        if states not in ([], ["INTENT"], ["INTENT", "COMPLETED"], ["INTENT", "FAILED"]):
            raise DiagnosticExecutionStop("DIAGNOSTIC_JOURNAL_EVENT_GRAMMAR_DRIFT")
        if events:
            ...
            if (
                ...
                raise DiagnosticExecutionStop("DIAGNOSTIC_JOURNAL_INTENT_DRIFT")
        return tuple(events)
    ...
    def _audit_checkpoints(self) -> None:
        ...
        if not self._checkpoint_root.is_dir() or self._checkpoint_root.is_symlink():
            raise DiagnosticExecutionStop("DIAGNOSTIC_CHECKPOINT_ROOT_INVALID")
        for path in self._checkpoint_root.iterdir():
            if (
                ...
                raise DiagnosticExecutionStop("DIAGNOSTIC_CHECKPOINT_ORPHAN_OR_PATH_DRIFT")
    ...
    def _audit_prefix(self) -> None:
        ...
        for wire in self._wires:
            ...
            if not events:
                ...
                continue
            if saw_gap:
                raise DiagnosticExecutionStop("DIAGNOSTIC_JOURNAL_EXECUTION_PREFIX_DRIFT")
            if len(events) == 1:
                raise DiagnosticExecutionStop("DIAGNOSTIC_UNMATCHED_INTENT_STOP")
            if events[-1]["state"] == "FAILED":
                raise DiagnosticExecutionStop("DIAGNOSTIC_FAILED_REQUEST_STOP")
            detail = events[0]["detail"]
            ...
            if (
                ...
                raise DiagnosticExecutionStop("DIAGNOSTIC_PACING_JOURNAL_DRIFT")
    ...
    def append(
        ...
        if (not events and state != "INTENT") or (
            ...
            raise DiagnosticExecutionStop("DIAGNOSTIC_JOURNAL_INVALID_TRANSITION")
        ordinal = next(
            index
            for index, item in enumerate(self._wires, 1)
            if item.request_identity == wire.request_identity
        ...
        if type(recorded) not in {int, float} or not math.isfinite(recorded) or recorded < 0:
            raise DiagnosticExecutionStop("DIAGNOSTIC_WALL_CLOCK_INVALID")
        body: dict[str, object] = {
        ...
        try:
            ...
            raise DiagnosticExecutionStop("DIAGNOSTIC_JOURNAL_EVENT_CREATE_RACE") from error
        return body
    ...
    def write_checkpoint(self, wire: ProviderWireRequest, body: bytes) -> str:
        ...
        if path.exists():
            raise DiagnosticExecutionStop("DIAGNOSTIC_CHECKPOINT_ALREADY_EXISTS")
        try:
            _atomic_bytes_create(path, body)
        except FileExistsError as error:
            raise DiagnosticExecutionStop("DIAGNOSTIC_CHECKPOINT_CREATE_RACE") from error
        return relative
    ...
    def replay(
        ...
        if completed_event.get("state") != "COMPLETED":
            raise DiagnosticExecutionStop("DIAGNOSTIC_COMPLETED_EVENT_REQUIRED")
        detail = completed_event.get("detail")
        if not isinstance(detail, dict):
            raise DiagnosticExecutionStop("DIAGNOSTIC_COMPLETED_DETAIL_INVALID")
        raw_relative = detail.get("checkpointPath")
        if type(raw_relative) is not str:
            raise DiagnosticExecutionStop("DIAGNOSTIC_CHECKPOINT_PATH_INVALID")
        ...
        if (
            ...
            raise DiagnosticExecutionStop("DIAGNOSTIC_UNSAFE_CHECKPOINT_PATH")
        path = (self._run_root / relative).resolve()
        try:
            path.relative_to(self._run_root)
        except ValueError as error:
            raise DiagnosticExecutionStop("DIAGNOSTIC_UNSAFE_CHECKPOINT_PATH") from error
        if not path.is_file() or path.is_symlink():
            raise DiagnosticExecutionStop("DIAGNOSTIC_CHECKPOINT_MISSING")
        ...
        if not isinstance(raw_headers, list) or any(
            ...
            raise DiagnosticExecutionStop("DIAGNOSTIC_REPLAY_HEADERS_INVALID")
        response = TransportResponse(
        ...
        if detail != expected:
            raise DiagnosticExecutionStop("DIAGNOSTIC_COMPLETED_DETAIL_DRIFT")
        return response
...
def _result_body(value: DiagnosticExecutionResult, *, include_hash: bool) -> dict[str, object]:
    body: dict[str, object] = {
...
def git_safe_execution_summary_v1(value: DiagnosticExecutionResult) -> dict[str, object]:
    """Return a hash-only summary that cannot contain provider response bytes."""

    validate_execution_result_v1(value)
    return _result_body(value, include_hash=True)


def validate_execution_result_v1(value: DiagnosticExecutionResult) -> None:
    if type(value) is not DiagnosticExecutionResult:
        raise DiagnosticExecutionStop("DIAGNOSTIC_EXECUTION_RESULT_TYPE_INVALID")
    if (
        type(value.run_id) is not str
        or _RUN_ID.fullmatch(value.run_id) is None
        or value.run_id == PREDECESSOR_CANARY_RUN_ID
        ...
        raise DiagnosticExecutionStop("DIAGNOSTIC_EXECUTION_RESULT_BINDING_DRIFT")
    for response, body_hash, header_hash in zip(
        ...
        if binding["bodySha256"] != body_hash or binding["responseHeadersHash"] != header_hash:
            raise DiagnosticExecutionStop("DIAGNOSTIC_EXECUTION_RESULT_RESPONSE_DRIFT")
    if value.content_hash != canonical_hash(_result_body(value, include_hash=False)):
        raise DiagnosticExecutionStop("DIAGNOSTIC_EXECUTION_RESULT_HASH_DRIFT")


def _lease_id(plan: DiagnosticPlan, authorization: PhaseAuthorization) -> str:
...
def _checkpoint_receipt_set_body(
    ...
    return [
        ...
        for wire, response, event_hash in zip(
            wires, responses, terminal_event_hashes, strict=True
        )
    ]


def _replay_verification_body(
    value: DiagnosticReplayVerification, *, include_hash: bool
) -> dict[str, object]:
    body: dict[str, object] = {
        "executionContractVersion": EXECUTION_CONTRACT_VERSION,
        "runId": value.run_id,
...
def _storage_acceptance_body(
    value: StorageBackedDiagnosticAcceptance, *, include_hash: bool
) -> dict[str, object]:
    body: dict[str, object] = {
        "executionContractVersion": EXECUTION_CONTRACT_VERSION,
        "runId": value.run_id,
...
def validate_replay_verification_v1(value: DiagnosticReplayVerification) -> None:
    if type(value) is not DiagnosticReplayVerification:
        raise DiagnosticExecutionStop("DIAGNOSTIC_REPLAY_VERIFICATION_TYPE_INVALID")
    if (
        type(value.run_id) is not str
        or _RUN_ID.fullmatch(value.run_id) is None
        or value.run_id == PREDECESSOR_CANARY_RUN_ID
        ...
        raise DiagnosticExecutionStop("DIAGNOSTIC_REPLAY_VERIFICATION_DRIFT")


def verify_diagnostic_review_from_storage_v1(
    plan: DiagnosticPlan,
    authorization: PhaseAuthorization,
    expected_review: DiagnosticReview,
    *,
    storage_root: Path,
    wall_clock: Callable[[], float] = time.time,
) -> tuple[DiagnosticReview, DiagnosticReplayVerification]:
    """Rebuild and compare the review from exact private persisted responses."""

    _validate_plan(plan)
    validate_phase_authorization_v1(plan, authorization)
    if authorization.network_authorized is not True:
        raise DiagnosticExecutionStop("DIAGNOSTIC_NETWORK_NOT_AUTHORIZED")
    wires = build_diagnostic_wire_requests_v1(plan)
    run_root = diagnostic_run_root_v1(storage_root, authorization)
    if (
        not run_root.is_dir()
        or run_root.is_symlink()
        or not (run_root / "plan-authorization.json").is_file()
    ):
        raise DiagnosticExecutionStop("DIAGNOSTIC_COMPLETED_RUN_NOT_FOUND")
    try:
        with ExecutionLease(
            run_root / ".lock",
            ...
            for wire in wires:
                ...
                if len(events) != 2 or events[-1]["state"] != "COMPLETED":
                    raise DiagnosticExecutionStop("DIAGNOSTIC_COMPLETED_RUN_INCOMPLETE")
                responses.append(journal.replay(wire, events[-1]))
                terminal_hashes.append(str(events[-1]["eventHash"]))
    except DiagnosticExecutionStop:
        raise
    except RuntimeError as error:
        raise DiagnosticExecutionStop(str(error)) from error
    response_tuple = tuple(responses)
    terminal_tuple = tuple(terminal_hashes)
    try:
        replay_review = build_diagnostic_review_v1(plan, response_tuple)
    except (DiagnosticStop, TypeError, ValueError) as error:
        raise DiagnosticExecutionStop("DIAGNOSTIC_STORAGE_REVIEW_REBUILD_FAILED") from error
    if type(expected_review) is not DiagnosticReview or replay_review != expected_review:
        raise DiagnosticExecutionStop("DIAGNOSTIC_STORAGE_REVIEW_REPLAY_DRIFT")
    ...
    provisional = DiagnosticReplayVerification(
    ...
    verification = DiagnosticReplayVerification(
        **{
            **asdict(provisional),
    ...
    validate_replay_verification_v1(verification)
    return replay_review, verification


def seal_storage_backed_diagnostic_acceptance_v1(
    plan: DiagnosticPlan,
    authorization: PhaseAuthorization,
    expected_review: DiagnosticReview,
    *,
    ...
    DiagnosticReplayVerification,
    DiagnosticAcceptance,
    StorageBackedDiagnosticAcceptance,
]:
    ...
    if accepted is False and decision_code != REJECTED_DECISION_CODE:
        raise DiagnosticExecutionStop("DIAGNOSTIC_REJECTED_DECISION_CODE_INVALID")
    try:
        diagnostic_acceptance = seal_diagnostic_acceptance_v1(
            plan,
            replay_review,
            accepted=accepted,
        ...
        raise DiagnosticExecutionStop("DIAGNOSTIC_STORAGE_ACCEPTANCE_REJECTED") from error
    provisional = StorageBackedDiagnosticAcceptance(
        run_id=authorization.run_id,
        plan_content_hash=plan.content_hash,
        authorization_content_hash=authorization.content_hash,
    ...
    storage_acceptance = StorageBackedDiagnosticAcceptance(
        **{
            **asdict(provisional),
            "content_hash": canonical_hash(
...
def validate_storage_backed_diagnostic_acceptance_v1(
    plan: DiagnosticPlan,
    authorization: PhaseAuthorization,
    expected_review: DiagnosticReview,
    verification: DiagnosticReplayVerification,
    diagnostic_acceptance: DiagnosticAcceptance,
    storage_acceptance: StorageBackedDiagnosticAcceptance,
    *,
    storage_root: Path,
) -> None:
    ...
    replay_review, replay_verification = verify_diagnostic_review_from_storage_v1(
        plan,
        authorization,
        expected_review,
        storage_root=storage_root,
    ...
    try:
        validate_diagnostic_acceptance_v1(
            plan, replay_review, diagnostic_acceptance
        )
    except (DiagnosticStop, TypeError, ValueError) as error:
        raise DiagnosticExecutionStop("DIAGNOSTIC_ACCEPTANCE_REPLAY_DRIFT") from error
    if type(storage_acceptance) is not StorageBackedDiagnosticAcceptance:
        raise DiagnosticExecutionStop("DIAGNOSTIC_STORAGE_ACCEPTANCE_TYPE_INVALID")
    if (
        ...
        or not _is_upper_sha256(
            storage_acceptance.replay_verification_content_hash
        )
        ...
        or not _is_upper_sha256(
            storage_acceptance.diagnostic_acceptance_content_hash
        )
        ...
        != canonical_hash(
            _storage_acceptance_body(storage_acceptance, include_hash=False)
        )
    ):
        raise DiagnosticExecutionStop("DIAGNOSTIC_STORAGE_ACCEPTANCE_DRIFT")


def _monotonic_micros(clock: Callable[[], float]) -> int:
    value = clock()
    if type(value) not in {int, float} or not math.isfinite(value) or value < 0:
        raise DiagnosticExecutionStop("DIAGNOSTIC_MONOTONIC_CLOCK_INVALID")
    return int(value * 1_000_000)


def execute_openfigi_diagnostic_v14(
    plan: DiagnosticPlan,
    authorization: PhaseAuthorization,
    *,
    storage_root: Path,
    ...
    """Execute or exactly replay the two-request diagnostic under one lease."""
    ...
    if type(wires) is not tuple or len(wires) != PHYSICAL_REQUEST_COUNT:
        raise DiagnosticExecutionStop("DIAGNOSTIC_WIRE_SET_DRIFT")
    for wire in wires:
        _wire_binding(wire)
    ...
    if not authorization.test_only and (
        ...
        raise DiagnosticExecutionStop("DIAGNOSTIC_PRODUCTION_CLOCK_INJECTION_BLOCKED")
    run_root = diagnostic_run_root_v1(storage_root, authorization)
    approved_storage_root = _validated_storage_root(
            ...
            for wire in wires:
                ...
                if previous is not None:
                    ...
                    if now - previous[1] < OPENFIGI_PACING_INTERVAL_MICROS:
                        raise DiagnosticExecutionStop("DIAGNOSTIC_PACING_INTERVAL_NOT_MET")
                ...
                if replayed_response != response:
                    raise DiagnosticExecutionStop("DIAGNOSTIC_POST_WRITE_REPLAY_DRIFT")
    ...
    if len(response_tuple) != PHYSICAL_REQUEST_COUNT:
        raise DiagnosticExecutionStop("DIAGNOSTIC_RESPONSE_SET_INCOMPLETE")
    body_hashes = tuple(_sha256_bytes(item.body) for item in response_tuple)
    header_hashes = tuple(
        canonical_hash([list(header) for header in _validated_headers(item.headers)])
        for item in response_tuple
    )
    provisional = DiagnosticExecutionResult(
        run_id=authorization.run_id,
        plan_content_hash=plan.content_hash,
        authorization_content_hash=authorization.content_hash,
        physical_request_count=PHYSICAL_REQUEST_COUNT,
    ...
    validate_execution_result_v1(result)
    return result
