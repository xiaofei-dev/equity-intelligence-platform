# Documentation Index

## Product and Scope

- [Product Vision](product-vision.md)
- [MVP Scope](mvp-scope.md)
- [Roadmap](roadmap.md)

## Engineering

- [System Architecture](architecture.md)
- [Local Development](development.md)
- [Data and Backtesting](data-and-backtesting.md)
- [User and Portfolio Data Model](user-portfolio-data-model.md)
- [Quantitative Screening Design](quantitative-screening.md)
- [Quantitative Screening v1 Specification and Data Acceptance Plan](quantitative-screening-v1-specification.md)
- [Data Source Validation Matrix](data-source-validation-matrix.md)
- [Provider Acceptance Report: 2026-07-26](provider-acceptance-report-2026-07-26.md)
- [Screening Internal API Contract v1](screening-api-contract.md)
- [Objective Rating v1 Validation Report](objective-rating-v1-validation.md)
- [Forward Decision-Quality Validation v1](forward-decision-quality-validation-v1.md)
- [Forward Validation Internal API Contract v1](forward-validation-api-contract.md)
- [One-Month Forward Validation Report Template](forward-validation-one-month-report-template.md)
- [Two-Month Forward Validation Report Template](forward-validation-two-month-report-template.md)
- [AI Analysis](ai-analysis.md)
- [Frontend Module](../frontend/README.md)
- [Spring Boot Module](../backend-java/README.md)
- [Analytics Module](../analysis-python/README.md)
- [Database Assets](../database/README.md)

## Investment and Governance

- [Investment Methodology](investment-methodology.md)
- [Decision Log](decision-log.md)
- [Development Log](development-log/README.md)

Product or architecture changes must be reflected in the relevant document and
appended to the decision log. Implementation milestones, verification evidence,
and unresolved issues belong in the dated development log.
